...
export default function DashboardPage() {
  ...
  async function loadContent() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/content?repo=${encodeURIComponent(repoUrl.trim())}`);
  ...
  async function handlePublish() {
    ...
    try {
      ...
      const refreshRes = await fetch(`/api/content?repo=${encodeURIComponent(repoUrl.trim())}`);
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 max-w-4xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-lg font-semibold text-zinc-100">CMS — Édition du site</h1>
      ...
      {!content ? (
        <div className="rounded-xl bg-zinc-900 border border-zinc-800 p-6">
          <h2 className="text-base font-medium text-zinc-200 mb-4">Charger un projet</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-zinc-400 mb-1">URL du dépôt GitHub (ex. <code className="text-zinc-500">owner/repo</code> ou URL complète)</label>
              <input
                type="text"
                value={repoUrl}
            ...
            <div>
              <label className="block text-sm text-zinc-400 mb-1">URL du site (optionnel — pour ouvrir le site en production)</label>
              <input
                type="url"
                value={siteUrl}
            ...
            <button
              ...
              onClick={loadContent}
              disabled={loading}
              className="rounded-lg bg-amber-600 px-4 py-2 font-medium text-zinc-950 hover:bg-amber-500 disabled:opacity-50"
            >
              {loading ? "Chargement…" : "Charger le contenu"}
            </button>
          </div>
        </div>
      ) : (
        <>
          <div className="flex flex-wrap items-center gap-3 mb-6">
            ...
            <button
              type="button"
              onClick={() => { setContent(null); setLoadError(""); }}
              className="text-sm text-zinc-400 hover:text-zinc-200"
            >
              Changer de projet
            </button>
